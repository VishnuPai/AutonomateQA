...
- **Environment selection**: Choose an environment in the UI; the target URL is filled from that environment’s **BaseUrl** and test data (e.g. credentials) is loaded from that environment’s CSV. Execution history and result details show which environment was used. **Feature-specific CSV**: when you run a feature with an environment selected, the runner looks for a CSV named `TestData/{FeatureName}.{Environment}.{ApplicationName}.csv`, where **ApplicationName** is set in **TestData:ApplicationName** in config (e.g. so that `Login.SIT.MyApp.csv` is used when FeatureName=Login, Environment=SIT, ApplicationName=MyApp). If that file exists it is used for that run; otherwise it tries `TestData/{FeatureName}.{Environment}.csv`, then falls back to the environment’s default CSV. **Re-run** uses the original run’s environment and that environment’s CSV when available.
...
4. **Secrets and test data**
   ...
   - **Per-environment test data**: Use the **Environment** dropdown in the UI; each environment’s **Environments:{Env}:CsvPath** in `appsettings.json` points to a default CSV for that env. Set **TestData:ApplicationName** (e.g. to your application or product name) so that feature-specific CSVs follow the pattern `TestData/{FeatureName}.{Environment}.{ApplicationName}.csv` (e.g. `Login.SIT.MyApp.csv` when ApplicationName is "MyApp"). When that file exists, it is used for runs of that feature in that environment instead of the default. If **ApplicationName** is not set, the runner tries `TestData/{FeatureName}.{Environment}.csv`. See [ENVIRONMENT_SETUP.md](ENVIRONMENT_SETUP.md).
...
| **TestData:ApplicationName** | Optional. Application or product name used in feature-specific CSV resolution: `TestData/{FeatureName}.{Environment}.{ApplicationName}.csv`. When set, the runner looks for that file first; when not set, it tries `TestData/{FeatureName}.{Environment}.csv`. | `TestData__ApplicationName` |
