...
9. MODALS / DIALOGS / POPUPS: When the step asserts a final page state (e.g. ""redirected to authorized page"", ""logged in"", ""on the home page"", ""I was redirected""), return Passed=false if the snapshot shows an open modal, dialog, or popup (e.g. role ""dialog"", ""alertdialog"", or a prominent overlay blocking the main content). The expected state means the main content is visible and no unexpected dialog is open. If the step explicitly verifies a modal or popup (e.g. ""popup is shown"", ""dialog is displayed""), then apply normal matching for that assertion instead.
